...
export const ProjectCard: React.FC<ProjectCardProps> = ({ project, onSelect, index }) => {
  ...
  return (
    <motion.div
      ...
      className={`group relative rounded-2xl bg-[#242424] border border-[#F7F6F4]/10 p-6 sm:p-8 flex flex-col justify-between overflow-hidden transition-all duration-300 hover:border-[#802938]/50 hover:bg-[#272727] ${
    ...
    >
      ...
      <div className="relative z-10">
        ...
        <h3
          ...
          className="text-2xl sm:text-3xl font-semibold text-[#F7F6F4] font-display tracking-tight group-hover:text-[#F7F6F4] cursor-pointer transition-colors"
        >
          {project.title}
        </h3>

        <p className="text-xs font-mono text-[#D6D2CB] mt-1 mb-4 opacity-80">
          {project.tagline}
        </p>

        <p className="text-sm text-[#D6D2CB] leading-relaxed mb-6 font-normal">
          {project.description}
        </p>

        {/* Featured Project Metrics / Highlights */}
        {isFeatured && project.metrics && (
          <div className="mb-6 grid grid-cols-3 gap-3 p-3.5 rounded-xl bg-[#1E1E1E]/80 border border-[#F7F6F4]/5">
      ...
      <div className="relative z-10 pt-6 border-t border-[#F7F6F4]/10 mt-auto">
        ...
        <button
          type="button"
          onClick={() => onSelect(project)}
          className="w-full flex items-center justify-between text-xs font-mono text-[#D6D2CB] hover:text-[#802938] transition-colors pt-2 group/btn"
        >
          <span>View Deep Dive &amp; Architecture</span>
          <ArrowRight className="w-3.5 h-3.5 transition-transform group-hover/btn:translate-x-1 text-[#802938]" />
        </button>
      </div>
    </motion.div>
  );
